...
class SpeakTextActionServer(Node):
    ...
    def goal_cb(self, goal_request):
        if not goal_request.text:
            self.get_logger().warn('Reject empty text goal')
    ...
    async def execute_cb(self, goal_handle):
        ...
        # Resolve parameters (override only if positive / meaningful)
        ...
        cache_key = (goal.text, speaker_id, speed, pitch, intonation, volume,
        ...
        if goal.allow_cache and cache_key in self._cache:
            ...
            try:
                query_resp = requests.post(
                    ...
                    params={'text': goal.text, 'speaker': speaker_id,
        ...
        if save_wav:
            ...
            path = Path(output_dir) / f'tts_{ts}.wav'
    ...
    async def _play_async(self, wav_bytes: bytes, goal_handle):
        try:
            ...
            self.get_logger().warn(f'simpleaudio failed: {e}')
